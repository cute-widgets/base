`<cute-slider>` allows for the selection of a value from a range via mouse, touch, or keyboard,
...
```html
<cute-slider min="1" max="5" step="0.5">
  <input cuteSliderThumb value="1.5">
</cute-slider>
```

### Selecting a range
A `<cute-slider>` can be converted into a range slider by projecting both a `cuteSliderStartThumb` and a
`cuteSliderEndThumb` into it. Each thumb has its own value, but both are still
constrained by the slider's `min` and `max` values. The `cuteSliderStartThumb` cannot have a value
greater than that of the `cuteSliderEndThumb` and the `cuteSliderEndThumb` cannot have a value less than
that of the `cuteSliderStartThumb`, though they both may have the same value.

```html
<cute-slider>
  <input cuteSliderStartThumb>
  <input cuteSliderEndThumb>
</cute-slider>
```
...
```html
<cute-slider discrete>
  <input cuteSliderThumb>
</cute-slider>
```
...
`CuteSlider` uses an internal `<input type="range">` to provide an accessible experience. The input
receives focus, and it can be labelled using `aria-label` or `aria-labelledby`.

Make sure the colors of the active and inactive track of the `CuteSlider` meet
