`<cute-select>` is a form control for selecting a value from a set of options, similar to the native
...
inside of a `<cute-form-field>` element.

To add options to the select, add `<cute-option>` elements to the `<cute-select>`. Each `<cute-option>`
has a `value` property that can be used to set the value that will be selected if the user chooses
this option. The content of the `<cute-option>` is what will be shown to the user.

CuteWidgets also supports use of the native `<select>` element inside of
`<cute-form-field>`. The native control has several performance, accessibility,
...
To use a native select inside `<cute-form-field>`, import `CuteInputModule` and add the
`cuteNativeControl` attribute to the `<select>` element.

```html
<h4>Basic cute-select</h4>
<cute-form-field>
  <cute-label>Favorite food</cute-label>
  <cute-select>
    @for (food of foods; track food) {
      <cute-option [value]="food.value">{{food.viewValue}}</cute-option>
    }
  </cute-select>
</cute-form-field>
```
...
The `<cute-select>` supports 2-way binding to the `value` property without the need for Angular
forms.

<!-- example(select-value-binding) -->

Both`<cute-select>` and `<select>` support all of the form directives from the core `FormsModule` (`NgModel`) and
`ReactiveFormsModule` (`FormControl`, `FormGroup`, etc.) As with native `<select>`, `<cute-select>`
...
There are a number of `<cute-form-field>` features that can be used with both `<select>` and `<cute-select>`. These
...
The placeholder is text shown when the `<cute-form-field>` label is floating but the `<cute-select>`
is empty. It is used to give the user an additional hint about the value they should select. The
placeholder can be specified by setting the `placeholder` attribute on the `<cute-select>` element.
In some cases that `<cute-form-field>` may use the placeholder as the label (see the
...
disabled property on the `<select>` or `<cute-select>` and the `<option>` or `<cute-option>` elements respectively.
...
The `<cute-optgroup>` element can be used to group common options under a subheading. The name of the
group can be set using the `label` property of `<cute-optgroup>`. Like individual `<cute-option>`
elements, an entire `<cute-optgroup>` can be disabled or enabled by setting the `disabled` property
on the group.
...
`<cute-select>` defaults to single-selection mode, but can be configured to allow multiple selection
by setting the `multiple` property. This will allow the user to select multiple values at once. When
using the `<cute-select>` in multiple selection mode, its value will be a sorted list of all selected
values rather than a single value.

Using multiple selection with a native select element (`<select multiple>`) is discouraged
inside `<cute-form-field>`, as the inline listbox appearance is inconsistent with other
...
If you want to display a custom trigger label inside a `<cute-select>`, you can use the
`<cute-select-trigger>` element.

<!-- example(select-custom-trigger) -->

### Disabling the ripple effect

By default, when a user clicks on a `<cute-option>`, a ripple animation is shown. This can be disabled
by setting the `disableRipple` property on `<cute-select>`.

<!-- example(select-no-ripple) -->

### Adding custom styles to the dropdown panel

In order to facilitate easily styling the dropdown panel, `<cute-select>` has a `panelClass` property
...
The `<cute-form-field>` allows you to
[associate error messages](https://material.angular.io/components/form-field/overview#error-messages)
with your `<select>` or `<cute-select>`. By default, these error messages are shown when the control is invalid and
...
`<cute-select>`. The property takes an instance of an `ErrorStateMatcher` object. An
`ErrorStateMatcher` must implement a single method `isErrorState` which takes the `FormControl` for
this `<cute-select>` as well as the parent form and returns a boolean indicating whether errors
...
### Accessibility
When possible, prefer a native `<select>` element over `CuteSelect`. The native control
provides the most accessible experience across the widest range of platforms.

`CuteSelect` implements the combobox pattern detailed in the [1.2 version of the ARIA
...
in all browser and screen-reader combinations supported by `CuteWidgets`.
...
Always provide an accessible label for the select. This can be done by adding a `<cute-label>`
inside of `<cute-form-field>`, the `aria-label` attribute, or the `aria-labelledby` attribute.

By default, `CuteSelect` displays a checkmark to identify selected items. While you can hide the
...
This error is thrown if you attempt to bind the `multiple` property on `<cute-select>` to a dynamic
...
a `<cute-select multiple>`. For example, something like `mySelect.value = 'option1'`. What you likely
