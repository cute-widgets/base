...
export const FS_MATERIAL_ICONS = ['material-icons', 'cute-ligature-font'];
/** Bootstrap icons font CSS classes */
export const FS_BOOTSTRAP_ICONS = ['bi'];
/** FontAwesome icons font CSS classes */
export const FS_FONTAWESOME_ICONS = ['fa'];
/** FontAwesome solid icons font CSS classes */
export const FS_FONTAWESOME_SOLID = ['fa-solid'];
/** FontAwesome regular icons font CSS classes */
export const FS_FONTAWESOME_REGULAR = ['fa-regular'];
/** FontAwesome brand icons font CSS classes */
export const FS_FONTAWESOME_BRAND = ['fa-brand'];

/**
 * Returns an exception to be thrown in the case when attempting to
 * load an icon with a name that cannot be found.
 */
export function getCuteIconNameNotFoundError(iconName: string): Error {
 ...
 */
export function getCuteIconNoHttpProviderError(): Error {
    return Error(
        'Could not find HttpClient provider for use with CuteWidgets icons. ' +
 ...
 */
export function getCuteIconFailedToSanitizeUrlError(url: SafeResourceUrl): Error {
    return Error(
        `The URL provided to CuteIconRegistry was not trusted as a resource URL ` +
 ...
 */
export function getCuteIconFailedToSanitizeLiteralError(literal: SafeHtml): Error {
    return Error(
        `The literal provided to CuteIconRegistry was not trusted as safe HTML by ` +
...
export class CuteIconRegistry implements OnDestroy {
     ...
     */
    //private _defaultFontSetClass = FS_MATERIAL_ICONS;
    private _defaultFontSetClass = FS_BOOTSTRAP_ICONS;
    ...
    constructor(
      ...
      // Register Material Icons with two aliases: 'mi' and 'mat'.
      this.registerFontClassAlias("mi",   FS_MATERIAL_ICONS.join(" "));
      this.registerFontClassAlias("mat",  FS_MATERIAL_ICONS.join(" "));

      // Register FontAwesome icons
      this.registerFontClassAlias("fa",   FS_FONTAWESOME_ICONS.join(" "));
    }
    ...
    addSvgIconLiteralInNamespace(
        ...
        if (!cleanLiteral) {
            throw getCuteIconFailedToSanitizeLiteralError(literal);
        }
    ...
    getSvgIconFromUrl(safeUrl: SafeResourceUrl): Observable<SVGElement> {
        ...
        if (!url) {
            throw getCuteIconFailedToSanitizeUrlError(safeUrl);
        }
    ...
    getNamedSvgIcon(name: string, namespace: string = ''): Observable<SVGElement> {
        ...
        return observableThrow(getCuteIconNameNotFoundError(key));
    }
    ...
    private _getSvgFromIconSetConfigs(
        ...
        return forkJoin(iconSetFetchRequests).pipe(
            map(() => {
                ...
                // TODO: add an ngDevMode check
                if (!foundIcon) {
                    throw getCuteIconNameNotFoundError(name);
                }
    ...
    private _fetchIcon(iconConfig: SvgIconConfig): Observable<TrustedHTML> {
        ...
        if (!this._httpClient) {
            throw getCuteIconNoHttpProviderError();
        }
...
/** @docs-private */
export function CUTE_ICON_REGISTRY_PROVIDER_FACTORY(
    parentRegistry: CuteIconRegistry,
    httpClient: HttpClient,
    sanitizer: DomSanitizer,
    errorHandler: ErrorHandler,
    document?: any,
) {
    return parentRegistry || new CuteIconRegistry(httpClient, sanitizer, document, errorHandler);
}

/** @docs-private */
export const CUTE_ICON_REGISTRY_PROVIDER = {
    // If there is already a CuteIconRegistry available, use that. Otherwise, provide a new one.
    provide: CuteIconRegistry,
    deps: [
        [new Optional(), new SkipSelf(), CuteIconRegistry],
        [new Optional(), HttpClient],
        DomSanitizer,
        ErrorHandler,
        [new Optional(), DOCUMENT as InjectionToken<any>],
    ],
    useFactory: CUTE_ICON_REGISTRY_PROVIDER_FACTORY,
};
