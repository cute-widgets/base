 ...
 */
export class CuteDialogRef<T, R = any> {
  ...
  private _state = CuteDialogState.OPEN;

  // TODO(crisbeto): we shouldn't have to declare this property, because `DialogRef.close`
  // already has a second `options` parameter that we can use. The problem is that internal tests
  // have assertions like `expect(CuteDialogRef.close).toHaveBeenCalledWith(foo)` which will break,
  // because it'll be called with two arguments by things like `CuteDialogClose`.
  /** Interaction that caused the dialog to close. */
  private _closeInteractionType: FocusOrigin | undefined;

  constructor(
    private _ref: DialogRef<R, T>,
    private _config: CuteDialogConfig,
    public _containerInstance: CuteDialogContainer,
  ) {
...
export function _closeDialogVia<R>(ref: CuteDialogRef<R>, interactionType: FocusOrigin, result?: R) {
