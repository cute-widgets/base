`cute-grid-list` is a two-dimensional list view that arranges cells into grid-based layout.

<!-- example(grid-list-overview) -->

### Setting the number of columns

An `cute-grid-list` must specify a `cols` attribute which sets the number of columns in the grid. The
...
It is possible to set the rowspan and colspan of each `cute-grid-tile` individually, using the
`rowspan` and `colspan` properties. If not set, they both default to `1`. The `colspan` must not
exceed the number of `cols` in the `cute-grid-list`. There is no such restriction on the `rowspan`
however, more rows will simply be added for it the tile to fill.

### Tile headers and footers

A header and footer can be added to an `cute-grid-tile` using the `cute-grid-tile-header` and
`cute-grid-tile-footer` elements respectively.
