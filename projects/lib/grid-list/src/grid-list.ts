...
import {
  ...
  QueryList,
  ElementRef,
  ChangeDetectionStrategy, isDevMode, inject,
} from '@angular/core';
import {CuteGridTile} from './grid-tile';
...
@Component({
  selector: 'cute-grid-list',
  exportAs: 'cuteGridList',
...
export class CuteGridList implements CuteGridListBase, OnInit, AfterContentChecked, TileStyleTarget {
  ...
  private _tileCoordinator: TileCoordinator | undefined;
   ...
   */
  private _rowHeight: string = "1:1";

  /** The amount of space between tiles. This will be something like '5px' or '2em'. */
  private _gutter: string = '1px';

  /** Sets position and size styles for a tile */
  private _tileStyler: TileStyler | undefined;

  /** Query list of tiles that are being rendered. */
  @ContentChildren(CuteGridTile, {descendants: true}) _tiles!: QueryList<CuteGridTile>;
  ...
  private _checkCols() {
    if (!this.cols && isDevMode()) {
      throw Error(
        `cute-grid-list: must pass in number of columns. ` + `Example: <cute-grid-list cols="3">`,
      );
    }
  }
