...
optional config object. The `open` method will return an instance of `CuteBottomSheetRef`:
...
The `CuteBottomSheetRef` is a reference to the currently-opened bottom sheet and can be used to close
it or to subscribe to events. Note that only one bottom sheet can be open at a time. Any component
contained inside of a bottom sheet can inject the `CuteBottomSheetRef` as well.
...
import {CUTE_BOTTOM_SHEET_DATA} from '../bottom-sheet';
...
export class HobbitSheet {
  constructor(@Inject(CUTE_BOTTOM_SHEET_DATA) public data: {names: string[]}) { }
}
```

### Specifying global configuration defaults
Default bottom sheet options can be specified by providing an instance of `CuteBottomSheetConfig`
for `CUTE_BOTTOM_SHEET_DEFAULT_OPTIONS` in your application's root module.

```ts
@NgModule({
  providers: [
    {provide: CUTE_BOTTOM_SHEET_DEFAULT_OPTIONS, useValue: {hasBackdrop: false}}
  ]
})
```
...
the `disableClose` property of `CuteBottomSheetConfig`, doing this breaks the expected interaction
pattern for the ARIA `role="dialog"` pattern.

#### Focus management

When opened, `CuteBottomSheet` traps browser focus such that it cannot escape the root
...
`CuteBottomSheetConfig`, which supports the following values.
...
When closed, `CuteBottomSheet` restores focus to the element that previously held focus when the
