`cuteInput` is a directive that allows native `<input>` and `<textarea>` elements to work with
[`<cute-form-field>`](https://material.angular.io/components/form-field/overview).
...
on elements inside `<cute-form-field>` as well. This includes Angular directives such as `ngModel`
and `formControl`.
...
be used with `cuteNativeControl`:
* color
* date
* datetime-local
* email
...
There are a number of `<cute-form-field>` features that can be used with any `<input cuteNativeControl>` or
`<textarea cuteNativeControl>`. These include error messages, hint text, prefix & suffix, and theming. For
...
The placeholder is text shown when the `<cute-form-field>` label is floating but the input is empty.
...
element. In some cases that `<cute-form-field>` may use the placeholder as the label (see the
...
with your `cuteNativeControl`. By default, these error messages are shown when the control is invalid and
...
`cuteNativeControl`. The property takes an instance of an `ErrorStateMatcher` object. An `ErrorStateMatcher`
must implement a single method `isErrorState` which takes the `FormControl` for this `cuteNativeControl` as
...
If the containing `<cute-form-field>` has a label it will automatically be used as the `aria-label`
...
Any `cute-error` and `cute-hint` are automatically added to the input's `aria-describedby` list, and
...
#### Error: Input type "..." isn't supported by cuteInput

This error is thrown when you attempt to set an input's `type` property to a value that isn't
supported by the `cuteInput` directive. If you need to use an unsupported input type with
`<cute-form-field>` consider writing a
