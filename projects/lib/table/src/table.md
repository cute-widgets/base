The `cute-table` provides a **Bootstrap** styled data-table that can be used to display rows of
data.
...
with `cute-` instead of `cdk-`. For more information on the interface and a detailed look at how
...
#### 1. Write your cute-table and provide data

Begin by adding the `<table cute-table>` component to your template and passing in data.
...
```html
<table cute-table [dataSource]="myDataArray">
  ...
</table>
```
...
```html
<ng-container cuteColumnDef="score">
  <th cute-header-cell *cuteHeaderCellDef> Score </th>
  <td cute-cell *cuteCellDef="let user"> {{user.score}} </td>
</ng-container>
```
...
you can instead define your column using the `cute-text-column`. The following column definition is
equivalent to the one above.

```html
<cute-text-column name="score"></cute-text-column>
```

Check out the API docs and examples of the `cute-text-column` to see how you can customize the header
...
Then add `cute-header-row` and `cute-row` to the content of your `cute-table` and provide your
column list as inputs.

```html
<tr cute-header-row *cuteHeaderRowDef="columnsToDisplay"></tr>
<tr cute-row *cuteRowDef="let myRowData; columns: columnsToDisplay"></tr>
```
...
Although `Cute Widgets` provides a ready-made table `DataSource` class, `CuteTableDataSource`, you may
...
generated class is `cute-column-NAME`. For example, cells in a column named "symbol" can be targeted with the
selector `.cute-column-symbol`.
...
The `CuteTable` is focused on a single responsibility: efficiently render rows of data in a
...
For example, you can add sorting and pagination to the table by using `CuteSort` and `CutePaginator` and
...
the `Cute Widgets` library comes with a `CuteTableDataSource` that has already implemented
...
To paginate the table's data, add a `<cute-paginator>` after the table.

If you are using the `CuteTableDataSource` for your table's data source, simply provide the
`CutePaginator` to your data source. It will automatically listen for page changes made by the user
...
For more information on using and configuring the `<cute-paginator>`, check out the
[cute-paginator docs](https://material.angular.io/components/paginator/overview).

The `CutePaginator` is one provided solution to paginating your table's data, but it is not the only
option. In fact, the table can work with any custom pagination UI or strategy since the `CuteTable`
...
To add sorting behavior to the table, add the `cuteSort` directive to the table and add
`cute-sort-header` to each column header cell that should trigger sorting. Note that you have to import `CuteSortModule` in order to initialize the `cuteSort` directive (see [API docs](https://material.angular.io/components/sort/api)).

```html
<!-- Name Column -->
<ng-container cuteColumnDef="position">
  <th cute-header-cell *cuteHeaderCellDef cute-sort-header> Name </th>
  <td cute-cell *cuteCellDef="let element"> {{element.position}} </td>
</ng-container>
```

If you are using the `CuteTableDataSource` for your table's data source, provide the `CuteSort`
...
By default, the `CuteTableDataSource` sorts with the assumption that the sorted column's name
...
default data accessor on the `CuteTableDataSource`.

If you are not using the `CuteTableDataSource`, but instead implementing custom logic to sort your
data, listen to the sort's `(cuteSortChange)` event and re-order your data according to the sort state.
...
[cuteSort docs](https://material.angular.io/components/sort/overview).

The `CuteSort` is one provided solution to sorting your table's data, but it is not the only option.
In fact, the table can work with any custom sorting UI or strategy since the `CuteTable` and
its interface is not tied to any one specific implementation.

#### Filtering

`Cute Widgets` does not provide a specific component to be used for filtering the `CuteTable`
...
If you are using the `CuteTableDataSource`, simply provide the filter string to the
`CuteTableDataSource`. The data source will reduce each row data to a serialized form and will filter
...
If you want to show a message when not data matches the filter, you can use the `*cuteNoDataRow`
directive.
...
Right now there is no formal support for adding a selection UI to the table, but `Cute Widgets`
...
```html
<ng-container cuteColumnDef="select">
  <th cute-header-cell *cuteHeaderCellDef>
    <cute-checkbox (change)="$event ? toggleAllRows() : null"
                  [checked]="selection.hasValue() && isAllSelected()"
                  [indeterminate]="selection.hasValue() && !isAllSelected()">
    </cute-checkbox>
  </th>
  <td cute-cell *cuteCellDef="let row">
    <cute-checkbox (click)="$event.stopPropagation()"
...
```html
<ng-container cuteColumnDef="cost">
  <th cute-header-cell *cuteHeaderCellDef> Cost </th>
  <td cute-cell *cuteCellDef="let data"> {{data.cost}} </td>
  <td cute-footer-cell *cuteFooterCellDef> {{totalCost}} </td>
</ng-container>
...
<tr cute-header-row *cuteHeaderRowDef="columnsToDisplay"></tr>
<tr cute-row *cuteRowDef="let myRowData; columns: columnsToDisplay"></tr>
<tr cute-footer-row *cuteFooterRowDef="columnsToDisplay"></tr>
```
...
add a `sticky` input to the `cuteHeaderRowDef`.
...
When using the `multiTemplateDataRows` directive to support multiple rows for each data object, the context of `*cuteRowDef` is the same except that the `index` value is replaced by `dataIndex` and `renderIndex`.

### Accessibility

By default, `CuteTable` applies `role="table"`, assuming the table's contains primarily static
...
The `CuteTable` does not require that you use a native HTML table. Instead, you can use an
alternative approach that uses `display: flex` for the table's styles.

This alternative approach replaces the native table element tags with the `CuteTable` directive
selectors. For example, `<table cute-table>` becomes `<cute-table>`; `<tr cute-row>` becomes
`<cute-row>`. The following shows a previous example using this alternative template:

```html
<cute-table [dataSource]="dataSource">
  <!-- User name Definition -->
  <ng-container cuteColumnDef="username">
    <cute-header-cell *cuteHeaderCellDef> User name </cute-header-cell>
    <cute-cell *cuteCellDef="let row"> {{row.username}} </cute-cell>
  </ng-container>

  <!-- Age Definition -->
  <ng-container cuteColumnDef="age">
    <cute-header-cell *cuteHeaderCellDef> Age </cute-header-cell>
    <cute-cell *cuteCellDef="let row"> {{row.age}} </cute-cell>
  </ng-container>

  <!-- Title Definition -->
  <ng-container cuteColumnDef="title">
    <cute-header-cell *cuteHeaderCellDef> Title </cute-header-cell>
    <cute-cell *cuteCellDef="let row"> {{row.title}} </cute-cell>
  </ng-container>

  <!-- Header and Row Declarations -->
  <cute-header-row *cuteHeaderRowDef="['username', 'age', 'title']"></cute-header-row>
  <cute-row *cuteRowDef="let row; columns: ['username', 'age', 'title']"></cute-row>
</cute-table>
```
...
### Tables with `CuteRipple`

By default, `CuteTable` does not set up Cute Design ripples for rows. A ripple effect can be
added to table rows by using the `CuteRipple` directive from `@cute-widgets/base/core`. Due to
limitations in browsers, ripples cannot be applied native `th` or `tr` elements. The recommended
approach for setting up ripples is using the non-native `display: flex` variant of `CuteTable`.
