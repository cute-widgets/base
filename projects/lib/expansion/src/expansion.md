...
The `<cute-expansion-panel-header>` shows a summary of the panel content and acts
as the control for expanding and collapsing. This header may optionally contain an
`<cute-panel-title>` and an `<cute-panel-description>`, which format the content of the
...
<cute-expansion-panel>
  ...
  <ng-template cuteExpansionPanelContent>
...
`CuteExpansionPanel` imitates the experience of the native `<details>` and `<summary>` elements.
...
of `<cute-expansion-panel-header>`, including buttons and anchors.
