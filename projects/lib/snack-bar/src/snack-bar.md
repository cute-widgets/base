`CuteSnackBar` is a service for displaying snack-bar notifications.
...
In either case, a `CuteSnackBarRef` is returned. This can be used to dismiss the snackbar or to
receive notification of when the snackbar is dismissed. For simple messages with an action, the
`CuteSnackBarRef` exposes an observable for when the action is triggered.
If you want to close a custom snackbar that was opened via `openFromComponent`, from within the
component itself, you can inject the `CuteSnackBarRef`.
...
### Dismissal
A snackbar can be dismissed manually by calling the `dismiss` method on the `CuteSnackBarRef`
...
To access the data in your component, you have to use the `CUTE_SNACK_BAR_DATA` injection token:

```ts
import {Component, Inject} from '@angular/core';
import {CUTE_SNACK_BAR_DATA} from '@cute-widgets/base/snack-bar';
...
export class MessageArchivedComponent {
  constructor(@Inject(CUTE_SNACK_BAR_DATA) public data: string) { }
}
```
...
* `cuteSnackBarLabel` - Marks the text of the snackbar shown to users
* `cuteSnackBarActions` - Marks the container element containing any action buttons
* `cuteSnackBarAction` - Marks an individual action button
...
`CUTE_SNACK_BAR_DEFAULT_OPTIONS` injection token.

```ts
@NgModule({
  providers: [
    {provide: CUTE_SNACK_BAR_DEFAULT_OPTIONS, useValue: {duration: 2500}}
  ]
})
```
...
`CuteSnackBar` announces messages via an `aria-live` region. While announcements use the `polite`
setting by default, you can customize this by setting the `politeness` property of
`CuteSnackBarConfig`.

`CuteSnackBar` does not move focus to the snackbar element. Moving focus like this would disrupt
