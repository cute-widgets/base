...
The `cuteRipple` attribute directive defines an area in which a ripple animates on user interaction.

```html
<div [cuteRipple]="{color: myColor}">
  <ng-content></ng-content>
</div>
```

By default, a ripple is activated when the host element of the `cuteRipple` directive receives
...
Ripples can also be triggered programmatically by getting a reference to the `CuteRipple` directive
...
the `cuteRippleTrigger` option that expects a reference to an `HTMLElement`.

```html
<div>
  <div cuteRipple [cuteRippleTrigger]="trigger" class="my-ripple-container">
...
Ripples can be shown programmatically by getting a reference to the `CuteRipple` directive.

```ts
class MyComponent {

  /** Reference to the directive instance of the ripple. */
  @ViewChild(CuteRipple) ripple: CuteRipple;
...
Global ripple options can be specified by setting the `CUTE_RIPPLE_GLOBAL_OPTIONS` provider.
...
@NgModule({
  providers: [
    {provide: CUTE_RIPPLE_GLOBAL_OPTIONS, useValue: globalRippleConfig}
  ]
})
```
...
To change global ripple options at runtime, just inject the `CUTE_RIPPLE_GLOBAL_OPTIONS`
...
@NgModule({
  providers: [
    {provide: CUTE_RIPPLE_GLOBAL_OPTIONS, useExisting: AppGlobalRippleOptions},
  ]
})
