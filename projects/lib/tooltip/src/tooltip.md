The CuteWidgets tooltip provides a text label that is displayed when the user hovers
...
Based on the position in which the tooltip is shown, the `.cute-tooltip-panel` element will receive a
CSS class that can be used for style (e.g., to add an arrow). The possible classes are
`cute-tooltip-panel-above`, `cute-tooltip-panel-below`, `cute-tooltip-panel-left`,
`cute-tooltip-panel-right`.

<!-- example(tooltip-position) -->

To display the tooltip relative to the mouse or touch that triggered it, use the
`cuteTooltipPositionAtOrigin` input.
...
To add a delay before showing or hiding the tooltip, you can use the inputs `cuteTooltipShowDelay`
and `cuteTooltipHideDelay` to provide a delay time in milliseconds.
...
your options using the `CUTE_TOOLTIP_DEFAULT_OPTIONS` injection token.
...
To completely disable a tooltip, set `cuteTooltipDisabled`. While disabled, a tooltip will never be
shown.

### Accessibility

`CuteTooltip` adds an `aria-describedby` description that provides a reference
