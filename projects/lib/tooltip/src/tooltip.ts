...
import {
  ...
  ViewContainerRef,
  ViewEncapsulation,
  inject, isDevMode, Injector, afterNextRender,
  DOCUMENT
} from '@angular/core';
 ...
 */
export function getCuteTooltipInvalidPositionError(position: string) {
  return Error(`Tooltip position "${position}" is invalid.`);
}

/** Injection token that determines the scroll handling while a tooltip is visible. */
export const CUTE_TOOLTIP_SCROLL_STRATEGY = new InjectionToken<() => ScrollStrategy>(
  'cute-tooltip-scroll-strategy',
  {
    providedIn: 'root',
    factory: () => {
...
/** @docs-private */
export function CUTE_TOOLTIP_SCROLL_STRATEGY_FACTORY(overlay: Overlay): () => ScrollStrategy {
  return () => overlay.scrollStrategies.reposition({scrollThrottle: SCROLL_THROTTLE_MS});
}

/** @docs-private */
export const CUTE_TOOLTIP_SCROLL_STRATEGY_FACTORY_PROVIDER = {
  provide: CUTE_TOOLTIP_SCROLL_STRATEGY,
  deps: [Overlay],
  useFactory: CUTE_TOOLTIP_SCROLL_STRATEGY_FACTORY,
};

/** @docs-private */
export function CUTE_TOOLTIP_DEFAULT_OPTIONS_FACTORY(): CuteTooltipDefaultOptions {
  return {
    showDelay: 0,
    hideDelay: 0,
...
export const CUTE_TOOLTIP_DEFAULT_OPTIONS = new InjectionToken<CuteTooltipDefaultOptions>(
  'cute-tooltip-default-options',
  {
    providedIn: 'root',
    factory: CUTE_TOOLTIP_DEFAULT_OPTIONS_FACTORY,
  },
);

/** Default `cuteTooltip` options that can be overridden. */
export interface CuteTooltipDefaultOptions {
 ...
 */
@Directive({
  selector: '[cuteTooltip]',
  exportAs: 'cuteTooltip',
  host: {
    'class': 'cute-tooltip-trigger',
    '[class.cute-tooltip-disabled]': 'disabled',
  },
  standalone: true,
})
export class CuteTooltip implements OnDestroy, AfterViewInit {
  ...
  private _defaultOptions = inject<CuteTooltipDefaultOptions>(CUTE_TOOLTIP_DEFAULT_OPTIONS, {
    optional: true,
  });

  protected _overlayRef: OverlayRef | null = null;
  protected _tooltipInstance: TooltipComponent | null = null;

  private _portal: ComponentPortal<TooltipComponent> | undefined;
  private _viewInitialized = false;
  private _pointerExitEventsInitialized = false;
  private readonly _scrollStrategy = inject(CUTE_TOOLTIP_SCROLL_STRATEGY);
  private readonly _tooltipComponent = TooltipComponent;
  private _viewportMargin = 8;
  private _currentPosition: TooltipPosition = "below";
  private readonly _cssClassPrefix: string = 'cute';
  private _ariaDescriptionPending: boolean = false;
  private _dirSubscribed = false;

  /** Allows the user to define the position of the tooltip relative to the parent element */
  @Input('cuteTooltipPosition')
   ...
   * instead of outside the element bounding box.
   */
  @Input('cuteTooltipPositionAtOrigin')
  get positionAtOrigin(): boolean { return this._positionAtOrigin; }
  ...
  set disabled(value: BooleanInput) {
    ...
    if (this._disabled !== isDisabled) {
      ...
      // If the tooltip is disabled, hide immediately.
      if (isDisabled) {
        this.hide(0);
      } else {
   ...
   */
  @Input('cuteTooltipTouchGestures') touchGestures: TooltipTouchGestures = 'auto';
  ...
  private _touchstartTimeout: ReturnType<typeof setTimeout> | null = null;
  ...
  private _createOverlay(origin?: {x: number; y: number}): OverlayRef {
    ...
    // Create a connected position strategy that listens for scroll events to reposition.
  ...
  private _getOrigin(): {main: OriginConnectionPosition; fallback: OriginConnectionPosition} {
    ...
    if (position == 'above' || position == 'below') {
      ...
      throw getCuteTooltipInvalidPositionError(position);
    }
 ...
 */
@Component({
    selector: 'cute-tooltip-component',
...
export class TooltipComponent implements OnDestroy {
  ...
  tooltipClass: string | string[] | Set<string> | {[key: string]: any} | undefined;
  ...
  private readonly _showAnimation = 'cute-tooltip-show';

  /** Name of the hide animation and the class that toggles it. */
  private readonly _hideAnimation = 'cute-tooltip-hide';
 ...
 _cancelPendingAnimations() {
  ...
  private _toggleVisibility(isVisible: boolean) {
    ...
    if (this._animationsDisabled) {
      tooltip.classList.add('_cute-animation-noopable');
