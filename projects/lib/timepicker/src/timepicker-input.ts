...
import {DateAdapter, CUTE_DATE_FORMATS} from '@cute-widgets/base/core';
import {
  AbstractControl,
...
import {CuteTimepicker, CuteTimepickerConnectedInput} from './timepicker';
...
import {CUTE_INPUT_VALUE_ACCESSOR} from "@cute-widgets/base/input";

/**
 * Input that can be used to enter time and connect to a `cute-timepicker`.
 */
@Directive({
  selector: 'input[cuteTimepicker]',
  exportAs: 'cuteTimepickerInput',
  host: {
    'class': 'cute-timepicker-input',
    'role': 'combobox',
    'type': 'text',
    ...
    '[attr.cute-timepicker-id]': 'timepicker()?.panelId',
    '[disabled]': 'disabled()',
    '(blur)': '_handleBlur()',
  ...
  providers: [
    {
      ...
      useExisting: CuteTimepickerInput,
      multi: true,
    },
    {
    ...
    {
      provide: CUTE_INPUT_VALUE_ACCESSOR,
      useExisting: CuteTimepickerInput,
    },
  ],
})
export class CuteTimepickerInput<D>
    implements CuteTimepickerConnectedInput<D>, ControlValueAccessor, Validator, OnDestroy
{
  ...
  private _formField = inject(CUTE_FORM_FIELD, {optional: true});
  ...
  readonly openOnClick: InputSignalWithTransform<boolean, unknown> = input(true, {
    alias: 'cuteTimepickerOpenOnClick',
  ...
  constructor() {
    ...
    // form field to be clickable, if the timepicker is used in `cute-form-field`.
  ...
  private _assignUserSelection(selection: D | null, propagateToAccessor: boolean) {
    ...
    if (selection == null || !this._isValid(selection)) {
      ...
      // datepicker. If the user enters a valid time afterward, the datepicker's selection will
