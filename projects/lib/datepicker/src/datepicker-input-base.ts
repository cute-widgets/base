...
import {
  ...
  OnChanges,
  SimpleChanges,
  booleanAttribute, isDevMode, inject,
} from '@angular/core';
import {
  AbstractControl,
...
import {DateAdapter, CUTE_DATE_FORMATS, CuteDateFormats, ThemeColor} from '@cute-widgets/base/core';
 ...
 */
export class CuteDatepickerInputEvent<D, S = unknown> {
  /** The new value for the target datepicker input. */
  value: D | null;

  constructor(
    /** Reference to the datepicker input component that emitted the event. */
    public target: CuteDatepickerInputBase<S, D>,
...
/**
 * Partial representation of `CuteFormField` that is used for backwards-compatibility
 * between the legacy and non-legacy variants.
 */
export interface _CuteFormFieldPartial {
...
@Directive()
export abstract class CuteDatepickerInputBase<S, D = ExtractDateTypeFromSelection<S>>
  implements ControlValueAccessor, AfterViewInit, OnChanges, OnDestroy, Validator
{
  ...
  @Output() readonly dateChange: EventEmitter<CuteDatepickerInputEvent<D, S>> = new EventEmitter<
    CuteDatepickerInputEvent<D, S>
  >();

  /** Emits when an `input` event is fired on this `<input>`. */
  @Output() readonly dateInput: EventEmitter<CuteDatepickerInputEvent<D, S>> = new EventEmitter<
    CuteDatepickerInputEvent<D, S>
  >();
  ...
  private _parseValidator: ValidatorFn = (): ValidationErrors | null => {
    return this._lastValueValid
      ? null
      : {'cuteDatepickerParse': {'text': this._elementRef.nativeElement.value}};
  };
  ...
  private _minValidator: ValidatorFn = (control: AbstractControl): ValidationErrors | null => {
    ...
    return !min || !controlValue || this._dateAdapter.compareDate(min, controlValue) <= 0
      ? null
      : {'cuteDatepickerMin': {'min': min, 'actual': controlValue}};
  };
  ...
  private _maxValidator: ValidatorFn = (control: AbstractControl): ValidationErrors | null => {
    ...
    return !max || !controlValue || this._dateAdapter.compareDate(max, controlValue) >= 0
      ? null
      : {'cuteDatepickerMax': {'max': max, 'actual': controlValue}};
  };
  ...
  _registerModel(model: CuteDateSelectionModel<S, D>): void {
    ...
    this._valueChangesSubscription = this._model.selectionChanged.subscribe((event) => {
      if (this._shouldHandleChangeEvent(event)) {
        ...
        this.dateInput.emit(new CuteDatepickerInputEvent(this, this._elementRef.nativeElement));
        this.dateChange.emit(new CuteDatepickerInputEvent(this, this._elementRef.nativeElement));
      }
    });
  }
