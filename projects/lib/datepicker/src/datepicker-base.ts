...
import {
  ...
  createGlobalPositionStrategy, createFlexibleConnectedPositionStrategy,
...
import {DateAdapter} from '@cute-widgets/base/core';
import {ThemeColor} from '@cute-widgets/base/core';
import {merge, Subject, Observable, Subscription} from 'rxjs';
import {filter, take} from 'rxjs/operators';
import {_getFocusedElementPierceShadowDom} from '@angular/cdk/platform';
import {CuteCalendar, CuteCalendarView} from './calendar';
import {createMissingDateImplError} from './datepicker-errors';
import {CuteCalendarUserEvent, CuteCalendarCellClassFunction} from './calendar-body';
import {DateFilterFn} from './datepicker-input-base';
import {
  ExtractDateTypeFromSelection,
  CuteDateSelectionModel,
  DateRange,
} from './date-selection-model';
import {
  CUTE_DATE_RANGE_SELECTION_STRATEGY,
  CuteDateRangeSelectionStrategy,
...
export const CUTE_DATEPICKER_SCROLL_STRATEGY = new InjectionToken<() => ScrollStrategy>(
  'cute-datepicker-scroll-strategy',
  {
    providedIn: 'root',
    factory: () => {
 ...
 */
@Component({
    selector: 'cute-datepicker-content',
    ...
    exportAs: 'cuteDatepickerContent',
    encapsulation: ViewEncapsulation.None,
    changeDetection: ChangeDetectionStrategy.OnPush,
    imports: [CdkTrapFocus, CuteCalendar, CdkPortalOutlet, CuteButton]
})
export class CuteDatepickerContent<S, D = ExtractDateTypeFromSelection<S>>
  implements AfterViewInit, OnDestroy
{
  protected _elementRef = inject(ElementRef);
  protected _animationsDisabled = _animationsDisabled();
  private _changeDetectorRef = inject(ChangeDetectorRef);
  private _globalModel = inject<CuteDateSelectionModel<S, D>>(CuteDateSelectionModel);
  private _dateAdapter = inject<DateAdapter<D>>(DateAdapter)!;
  private _ngZone = inject(NgZone);
  private _rangeSelectionStrategy = inject<CuteDateRangeSelectionStrategy<D>>(
    CUTE_DATE_RANGE_SELECTION_STRATEGY,
    {optional: true},
  );

  private _stateChanges: Subscription | undefined;
  private _model!: CuteDateSelectionModel<S, D>;
  private _eventCleanups: (() => void)[] | undefined;
  private _animationFallback: ReturnType<typeof setTimeout> | undefined;

  /** Reference to the internal calendar component. */
  @ViewChild(CuteCalendar) _calendar: CuteCalendar<D> | undefined;
  ...
  constructor() {
    this._closeButtonText = inject(CuteDatepickerIntl).closeCalendarLabel;
  ...
  _handleUserSelection(event: CuteCalendarUserEvent<D | null>) {
    ...
    // A null value when picking a range means that the user canceled the selection (e.g., by
  ...
  _handleUserDragDrop(event: CuteCalendarUserEvent<DateRange<D>>) {
    this._model.updateSelection(event.value as unknown as S, this);
  }

  _startExitAnimation() {
    this._elementRef.nativeElement.classList.add('cute-datepicker-content-exit');
  ...
  private _handleAnimationEvent = (event: AnimationEvent) => {
    ...
    if (event.target !== element || !event.animationName.startsWith('_cute-datepicker-content')) {
      return;
    }

    clearTimeout(this._animationFallback);
    this._isAnimating = event.type === 'animationstart';
    element.classList.toggle('cute-datepicker-content-animating', this._isAnimating);
...
export interface CuteDatepickerControl<D> {
...
export interface CuteDatepickerPanel<
  C extends CuteDatepickerControl<D>,
  S,
...
@Directive()
export abstract class CuteDatepickerBase<
  C extends CuteDatepickerControl<D>,
  S,
  D = ExtractDateTypeFromSelection<S>,
>
  implements CuteDatepickerPanel<C, S, D>, OnDestroy, OnChanges, Expandable
{
  private _injector = inject(Injector);
  private _viewContainerRef = inject(ViewContainerRef);
  private _dateAdapter = inject<DateAdapter<D>>(DateAdapter, {optional: true})!;
  private _dir = inject(Directionality, {optional: true});
  private _model = inject<CuteDateSelectionModel<S, D>>(CuteDateSelectionModel);
  private _animationsDisabled = _animationsDisabled();

  private _scrollStrategy = inject(CUTE_DATEPICKER_SCROLL_STRATEGY);
  ...
  get color(): ThemeColor|undefined {
    return (
      this._color || (this.datepickerInput ? this.datepickerInput.getThemeColor() : undefined)
    );
  }
   ...
   */
  @Output() readonly viewChanged: EventEmitter<CuteCalendarView> = new EventEmitter<CuteCalendarView>(
    true,
  );

  /** Function that can be used to add custom CSS classes to dates. */
  @Input() dateClass: CuteCalendarCellClassFunction<D> | undefined;
  ...
  id: string = `cute-datepicker-${datepickerUid++}`;
  ...
  private _componentRef: ComponentRef<CuteDatepickerContent<S, D>> | null = null;
  ...
  registerInput(input: C): CuteDateSelectionModel<S, D> {
    if (this.datepickerInput && isDevMode()) {
      throw Error('A CuteDatepicker can only be associated with a single input.');
    }
    this._inputStateChanges.unsubscribe();
  ...
  registerActions(portal: TemplatePortal): void {
    if (this._actionsPortal && isDevMode()) {
      throw Error('A CuteDatepicker can only be associated with a single actions row.');
    }
    this._actionsPortal = portal;
  ...
  open(): void {
    ...
    if (!this.datepickerInput && isDevMode()) {
      throw Error('Attempted to open an CuteDatepicker with no associated input.');
    }
  ...
  protected _forwardContentValues(instance: CuteDatepickerContent<S, D>) {
  ...
  private _openOverlay(): void {
    ...
    const portal = new ComponentPortal<CuteDatepickerContent<S, D>>(
    ...
    const overlayRef = (this._overlayRef = createOverlayRef(
      ...
      new OverlayConfig({
        ...
        hasBackdrop: true,
        backdropClass: [
          isDialog ? 'cdk-overlay-dark-backdrop' : 'cute-overlay-transparent-backdrop',
        ...
        panelClass: `cute-datepicker-${isDialog ? 'dialog' : 'popup'}`,
      }),
    ));
  ...
  private _getDropdownStrategy() {
    const strategy = createFlexibleConnectedPositionStrategy(
      ...
      .withTransformOriginOn('.cute-datepicker-content')
