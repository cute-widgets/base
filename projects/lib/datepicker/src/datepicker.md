...
A datepicker is composed of a text input and a calendar pop-up, connected via the `cuteDatepicker`
...
This works exactly the same with an input that is part of an `<cute-form-field>` and the toggle
can easily be used as a prefix or suffix on the Material input:

<!-- example({"example":"datepicker-overview",
              "file":"datepicker-overview-example.html"}) -->

If you want to customize the icon that is rendered inside the `cute-datepicker-toggle`, you can do so
by using the `cuteDatepickerToggleIcon` directive:
...
`cute-date-range-input` and `cute-date-range-picker` components. They work in tandem, similarly to the
`cute-datepicker` and the basic datepicker input.

The `cute-date-range-input` component requires two `input` elements for the start and end dates,
respectively:

```html
<cute-date-range-input>
  <input cuteStartDate placeholder="Start date">
  <input cuteEndDate placeholder="End date">
</cute-date-range-input>
```

The `cute-date-range-picker` component acts as the pop-up panel for selecting dates. This works in
the same way as `cute-datepicker`, but allows the user to select multiple times:

```html
<cute-date-range-picker #picker></cute-date-range-picker>
```

Connect the range picker and range input using the `rangePicker` property:

```html
<cute-date-range-input [rangePicker]="picker">
  <input cuteStartDate placeholder="Start date">
  <input cuteEndDate placeholder="End date">
</cute-date-range-input>
...
The `cute-date-range-input` component can be used together with the `FormGroup` directive from
...
The `startView` property of `<cute-datepicker>` can be used to set the view that will show up when
...
the `startAt` property of `<cute-datepicker>`. In this case the calendar will open to the month or
...
month. For example, if `<cute-datepicker>` is configured to work with javascript native Date
...
picker (if you're not familiar with the usage of `MomentDateAdapter` and `CUTE_DATE_FORMATS`
...
from the `cute-form-field` it is attached to. If you would like to specify a different palette for
the popup you can do so by setting the `color` property on `cute-datepicker`.
...
The second way to add date validation is using the `cuteDatepickerFilter` property of the datepicker
...
difference between using `cuteDatepickerFilter` vs using `min` or `max` is that filtering out all
...
* A value that violates the `min` property will have a `cuteDatepickerMin` error.
* A value that violates the `max` property will have a `cuteDatepickerMax` error.
* A value that violates the `cuteDatepickerFilter` property will have a `cuteDatepickerFilter` error.
...
`disabled` property. By default, the `<cute-datepicker>` and `<cute-datepicker-toggle>` will inherit
...
explicitly accept the value that they've selected, you can add a `<cute-datepicker-actions>` element
inside `<cute-datepicker>` with a "Cancel" and an "Apply" button marked with the
`cuteDatepickerCancel` and `cuteDatepickerApply` attributes respectively. Doing so will cause the
...
The actions element is also supported for `<cute-date-range-picker>` where that it is called
`<cute-date-range-picker-actions>` and the buttons are called `cuteDateRangePickerCancel` and
`cuteDateRangePickerApply` respectively.
...
you can provide the comparison range start and end dates to the `cute-date-range-input` using the
...
The `cute-date-range-picker` supports custom behaviors for range previews and selection. To customize
this, you first create a class that implements `CuteDateRangeSelectionStrategy`, and then provide
the class via the `CUTE_DATE_RANGE_SELECTION_STRATEGY` injection token. The following example
...
`<cute-datepicker>` has a `touchUi` property that can be set to `true` in order to enable a more
...
`<cute-datepicker>`. It also has an `opened` property that reflects the status of the popup.

<!-- example(datepicker-api) -->

### Using `cute-calendar` inline

If you want to allow the user to select a date from a calendar that is inlined on the page rather
than contained in a popup, you can use `<cute-calendar>` directly. The calendar's height is
...
These APIs include providers for `DateAdapter` and `CUTE_DATE_FORMATS`.
...
Because `DateAdapter` is a generic class, `CuteDatepicker` and `CuteDatepickerInput` also need to be
...
export class MyComponent {
  @ViewChild(CuteDatepicker) datepicker: CuteDatepicker<Date>;
}
```
...
`DateAdapter` implementation. You will also want to make sure that the `CUTE_DATE_FORMATS` provided
...
information about `CUTE_DATE_FORMATS`.

```ts
bootstrapApplication(MyApp, {
  providers: [
    {provide: DateAdapter, useClass: MyDateAdapter},
    {provide: CUTE_DATE_FORMATS, useValue: MY_DATE_FORMATS},
  ]
});
```
...
The `CUTE_DATE_FORMATS` object is a collection of formats that the datepicker uses when parsing
...
`calendarHeaderComponent` property of `<cute-datepicker>`. It takes a component class and constructs
...
`CuteCalendar` in the constructor. To make sure your header stays in sync with the calendar,
...
The various text strings used by the datepicker are provided through `CuteDatepickerIntl`.
...
bootstrapApplication(MyApp, {
  providers: [
    {provide: CuteDatepickerIntl, useClass: MyIntl},
...
The `CuteDatepicker` pop-up uses the `role="dialog"` interaction pattern. This dialog then contains
...
The `CuteDatepickerInput` and `CuteDatepickerToggle` directives both apply the `aria-haspopup`
attribute to the native input and button elements, respectively.

`CuteDatepickerIntl` includes strings that are used for `aria-label` attributes. Always provide
the datepicker text input a meaningful label via `<cute-label>`, `aria-label`, `aria-labelledby` or
`CuteDatepickerIntl`.

Always communicate the date format (e.g. 'MM/DD/YYYY'). This can be accomplished using `<cute-hint>`
or by providing an additional label adjacent to the form field.

`CuteDatepickerInput` adds <kbd>>Alt</kbd> + <kbd>Down Arrow</kbd> as a keyboard short to open the
...
additional means of opening the pop-up, such as `CuteDatepickerToggle`.
...
#### Error: A CuteDatepicker can only be associated with a single input

This error is thrown if more than one `<input>` tries to claim ownership over the same
`<cute-datepicker>` (via the `cuteDatepicker` attribute on the input). A datepicker can only be
associated with a single input.

#### Error: Attempted to open an CuteDatepicker with no associated input.

This error occurs if your `<cute-datepicker>` is not associated with any `<input>`. To associate an
input with your datepicker, create a template reference for the datepicker and assign it to the
`cuteDatepicker` attribute on the input:

```html
<input [cuteDatepicker]="picker">
<cute-datepicker #picker></cute-datepicker>
```
