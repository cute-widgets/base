...
import {IdGenerator} from "@cute-widgets/base/core";

/** Extra CSS classes that can be associated with a calendar cell. */
export type CuteCalendarCellCssClasses = string | string[] | Set<string> | {[key: string]: any};

/** Function that can generate the extra classes that should be added to a calendar cell. */
export type CuteCalendarCellClassFunction<D> = (
  date: D,
 ...
 * @docs-private
 */
export class CuteCalendarCell<D = any> {
  ...
  constructor(
    ...
    public cssClasses: CuteCalendarCellCssClasses = {},
...
export interface CuteCalendarUserEvent<D> {
  value: D;
  event: Event;
}
 ...
 */
@Component({
    selector: '[cute-calendar-body]',
    templateUrl: './calendar-body.html',
    styleUrl: './calendar-body.scss',
    host: {
        'class': 'cute-calendar-body',
    },
    exportAs: 'cuteCalendarBody',
    encapsulation: ViewEncapsulation.None,
    changeDetection: ChangeDetectionStrategy.OnPush,
    imports: [NgClass]
})
export class CuteCalendarBody<D = any> implements OnChanges, OnDestroy, AfterViewChecked {
  private _elementRef = inject<ElementRef<HTMLElement>>(ElementRef);
  private _ngZone = inject(NgZone);
  private _platform = inject(Platform);
  private _intl = inject(CuteDatepickerIntl);
  ...
  @Input() rows: CuteCalendarCell[][] = [];

  /** The value in the table that corresponds to today. */
  @Input() todayValue: number | undefined;
  ...
  @Input() labelMinRequiredCells: number = 0;
  ...
  @Output() readonly selectedValueChange = new EventEmitter<CuteCalendarUserEvent<number>>();

  /** Emits when the preview has changed as a result of a user action. */
  @Output() readonly previewChange = new EventEmitter<
    CuteCalendarUserEvent<CuteCalendarCell | null>
  >();

  @Output() readonly activeDateChange = new EventEmitter<CuteCalendarUserEvent<number>>();

  /** Emits the date at the possible start of a drag event. */
  @Output() readonly dragStarted = new EventEmitter<CuteCalendarUserEvent<D>>();

  /** Emits the date at the conclusion of a drag, or null if mouse was not released on a date. */
  @Output() readonly dragEnded = new EventEmitter<CuteCalendarUserEvent<D | null>>();

  /** The number of blank cells to put at the beginning for the first row. */
  protected _firstRowOffset: number = 0;
  ...
  protected _startDateLabelId: string | null = null

  /** ID for the end date label. */
  protected _endDateLabelId: string | null = null;

  /** ID for the comparison start date label. */
  protected _comparisonStartDateLabelId: string | null = null;

  /** ID for the comparison end date label. */
  protected _comparisonEndDateLabelId: string | null = null;
   ...
   */
  protected _trackRow = (row: CuteCalendarCell[]) => row;

  constructor(...args: unknown[]);
  constructor() {
    const idGenerator = inject(IdGenerator);
    this._startDateLabelId = idGenerator.getId('cute-calendar-body-start-');
    this._endDateLabelId = idGenerator.getId('cute-calendar-body-end-');
    this._comparisonStartDateLabelId = idGenerator.getId('cute-calendar-body-comparison-start-');
    this._comparisonEndDateLabelId = idGenerator.getId('cute-calendar-body-comparison-end-');
  ...
  protected _isSelected(value: number) {
  ...
  protected _isActiveCell(rowIndex: number, colIndex: number): boolean {
    ...
    if (rowIndex) {
      cellNumber -= this._firstRowOffset??0;
    }
  ...
  protected _isRangeStart(value: number) {
    return isStart(value, this.startValue, this.endValue);
  }

  /** Gets whether a value is the end of the main range. */
  protected _isRangeEnd(value: number) {
  ...
  protected _isComparisonStart(value: number) {
    return isStart(value, this.comparisonStart, this.comparisonEnd);
  }

  /** Whether the cell is a start bridge cell between the main and comparison ranges. */
  protected _isComparisonBridgeStart(value: number, rowIndex: number, colIndex: number) {
    if (!this._isComparisonStart(value) || this._isRangeStart(value) || !this._isInRange(value)) {
      return false;
    }

    let previousCell: CuteCalendarCell | undefined = this.rows[rowIndex][colIndex - 1];
  ...
  protected _isComparisonBridgeEnd(value: number, rowIndex: number, colIndex: number) {
    if (!this._isComparisonEnd(value) || this._isRangeEnd(value) || !this._isInRange(value)) {
      return false;
    }

    let nextCell: CuteCalendarCell | undefined = this.rows[rowIndex][colIndex + 1];
  ...
  protected _isComparisonEnd(value: number) {
    return isEnd(value, this.comparisonStart, this.comparisonEnd);
  }

  /** Gets whether a value is within the current comparison range. */
  protected _isInComparisonRange(value: number) {
   ...
   */
  protected _isComparisonIdentical(value: number) {
  ...
  protected _isPreviewStart(value: number) {
    return isStart(value, this.previewStart, this.previewEnd);
  }

  /** Gets whether a value is the end of the preview range. */
  protected _isPreviewEnd(value: number) {
    return isEnd(value, this.previewStart, this.previewEnd);
  }

  /** Gets whether a value is inside the preview range. */
  protected _isInPreview(value: number) {
  ...
  private _mouseupHandler = (event: Event) => {
    ...
    if (cellElement.closest('.cute-calendar-body') !== this._elementRef.nativeElement) {
  ...
  private _getCellFromElement(element: HTMLElement): CuteCalendarCell | null {
    const cell = getCellElement(element);

    if (cell) {
      const row = cell.getAttribute('data-cute-row');
      const col = cell.getAttribute('data-cute-col');
 ...
 * Or returns null if an element is not part of a date cell.
 */
function getCellElement(element: HTMLElement): HTMLElement | null {
  ...
  return cell?.getAttribute('data-cute-row') != null ? cell : null;
}
