`CuteWidgets`' stepper provides a wizard-like workflow by dividing content into logical steps.

`CuteWidgets`' stepper builds on the foundation of the CDK stepper that is responsible for the logic
that drives a stepped workflow. Stepper extends the CDK stepper and has `Bootstrap` styling.
...
For more complex labels, add a template with the `cuteStepLabel` directive inside the
`cute-step`.
...
#### Label position
For a horizontal `cute-stepper` it's possible to define the position of the label. `end` is the
default value, while `bottom` will place it under the step icon instead of at its side.
This behavior is controlled by `labelPosition` property.
...
`cuteStepperPrevious` and `cuteStepperNext`.
<!-- example({"example": "stepper-label-position-bottom",
              "file": "stepper-label-position-bottom-example.html",
              "region": "buttons"}) -->

### Linear stepper
The `linear` attribute can be set on `cute-stepper` to create a linear stepper that requires the
user to complete previous steps before proceeding to following steps. For each `cute-step`, the
...
#### Using a single form
When using a single form for the stepper, `cuteStepperPrevious` and `cuteStepperNext` have to be
set to `type="button"` in order to prevent submission of the form before all steps
are completed.

```html
<form [formGroup]="formGroup">
  <cute-stepper formArrayName="formArray" linear>
    <cute-step formGroupName="0" [stepControl]="formArray.get([0])">
      ...
      <div>
        <button cute-button cuteStepperNext type="button">Next</button>
      </div>
    </cute-step>
    <cute-step formGroupName="1" [stepControl]="formArray.get([1])">
      ...
      <div>
        <button cute-button cuteStepperPrevious type="button">Back</button>
        <button cute-button cuteStepperNext type="button">Next</button>
      </div>
    </cute-step>
    ...
...
#### Using a different form for each step
```html
<cute-stepper orientation="vertical" linear>
  <cute-step [stepControl]="formGroup1">
    <form [formGroup]="formGroup1">
      ...
    </form>
  </cute-step>
  <cute-step [stepControl]="formGroup2">
...
edit their responses. `editable="false"` can be set on `cute-step` to change the default.
...
set via `<cute-icon>` elements. If you want to provide a different set of icons, you can do so
by placing a `cuteStepperIcon` for each of the icons that you want to override. The `index`,
...
Note that you aren't limited to using the `cute-icon` component when providing custom icons.
...
an `ng-template` with the `cuteStepContent` attribute.
...
### Localizing labels
Labels used by the stepper are provided through `CuteStepperIntl`. Localization of these messages
can be done by providing a subclass with translated values in your application root module.

```ts
@NgModule({
  imports: [CuteStepperModule],
  providers: [
    {provide: CuteStepperIntl, useClass: MyIntl},
  ],
})
export class MyApp {}
```
...
`<cute-error>` with `<cute-form-field>`, or by using an ARIA live region.

When a step contains a forms validation error, `CuteStepper` will display the
