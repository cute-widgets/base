...
export class CuteAutocompleteSelectedEvent {
  constructor(
    /** Reference to the autocomplete panel that emitted the event. */
    public source: CuteAutocomplete,
    /** Option that was selected. */
    public option: CuteOption,
  ) {}
}

/** Event object that is emitted when an autocomplete option is activated. */
export interface CuteAutocompleteActivatedEvent {
  /** Reference to the autocomplete panel that emitted the event. */
...
export interface CuteAutocompleteDefaultOptions {
...
export const CUTE_AUTOCOMPLETE_DEFAULT_OPTIONS = new InjectionToken<CuteAutocompleteDefaultOptions>(
  'cute-autocomplete-default-options',
  {
    providedIn: 'root',
    factory: () => ({
...
@Component({
    selector: 'cute-autocomplete',
    templateUrl: './autocomplete.html',
    styleUrls: ['./autocomplete.scss'],
    encapsulation: ViewEncapsulation.None,
    changeDetection: ChangeDetectionStrategy.OnPush,
    exportAs: 'cuteAutocomplete',
    host: {
        'class': 'cute-autocomplete',
    },
    providers: [{ provide: CUTE_OPTION_PARENT_COMPONENT, useExisting: CuteAutocomplete }],
})
export class CuteAutocomplete implements AfterContentInit, OnDestroy {
  private _changeDetectorRef = inject(ChangeDetectorRef);
  private _elementRef = inject<ElementRef<HTMLElement>>(ElementRef);
  protected _defaults = inject<CuteAutocompleteDefaultOptions>(CUTE_AUTOCOMPLETE_DEFAULT_OPTIONS);
  ...
  _setColor(value: RichThemeColor | undefined) {
    this._color = value;
  ...
  /** Whether the active option should be selected as the user is navigating. */
  @Input({transform: booleanAttribute}) autoSelectActiveOption: boolean = false;
   ...
   */
  @Input({transform: booleanAttribute}) requireSelection: boolean = false;

  /**
   * Specify the width of the autocomplete panel.  Can be any CSS sizing value, otherwise it will
   * match the width of its host.
   */
  @Input() panelWidth: string | number | undefined;

  /** Whether ripples are disabled within the autocomplete panel. */
  @Input({transform: booleanAttribute}) disableRipple: boolean = false;

  /** Event that is emitted whenever an option from the list is selected. */
  @Output() readonly optionSelected: EventEmitter<CuteAutocompleteSelectedEvent> =
    new EventEmitter<CuteAutocompleteSelectedEvent>();
  ...
  @Output() readonly optionActivated: EventEmitter<CuteAutocompleteActivatedEvent> =
                                          new EventEmitter<CuteAutocompleteActivatedEvent>();
  ...
  id: string = `cute-autocomplete-${_uniqueAutocompleteIdCounter++}`;
