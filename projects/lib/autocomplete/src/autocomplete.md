...
defined by a `cute-option` tag. Set each option's value property to whatever you'd like the value
of the text input to be when that option is selected.

```html
<cute-autocomplete #auto>
  @for (option of filteredOptions | async; track option) {
    <cute-option [value]="option">{{option}}</cute-option>
  }
</cute-autocomplete>
```

Next, create the input and set the `cuteAutocomplete` input to refer to the template reference we assigned
...
to the input's `cuteAutocomplete` property.
...
filter criteria. This is especially helpful for screen reader users if you're using a non-standard
...
enable the `requireSelection` input on `cute-autocomplete`. This will change the behavior of
...
This behavior can be configured globally using the `CUTE_AUTOCOMPLETE_DEFAULT_OPTIONS`
injection token.
...
the panel, you can do so by setting the `autoActiveFirstOption` input on the `cute-autocomplete`
component. This behavior can be configured globally using the `CUTE_AUTOCOMPLETE_DEFAULT_OPTIONS`
injection token.
...
While `cute-autocomplete` supports attaching itself to a `cute-form-field`, you can also set it on
any other `input` element using the `cuteAutocomplete` attribute. This allows you to customize what
the input looks like without having to bring in the extra functionality from `cute-form-field`.
...
autocomplete is attached to using the `cuteAutocompleteOrigin` directive together with the
`cuteAutocompleteConnectedTo` input:

```html
<div class="custom-wrapper-example" cuteAutocompleteOrigin #origin="cuteAutocompleteOrigin">
  <input
    cuteInput
    [formControl]="myControl"
    [cuteAutocomplete]="auto"
    [cuteAutocompleteConnectedTo]="origin">
</div>
...
### Option groups
`cute-option` can be collected into groups using the `cute-optgroup` element:
<!-- example({"example":"autocomplete-optgroup",
              "file":"autocomplete-optgroup-example.html",
              "region":"cute-autocomplete"}) -->

### Accessibility

`CuteAutocomplete` implements the ARIA combobox interaction pattern. The text input trigger specifies
...
`<cute-label>` inside of `<cute-form-field>`, a native `<label>` element, the `aria-label`
attribute, or the `aria-labelledby` attribute.

`CuteAutocomplete` preserves focus on the text trigger, using `aria-activedescendant` to support
navigation though the autocomplete options.

By default, `CuteAutocomplete` displays a checkmark to identify the selected item. While you can hide
