...
import {
  ...
  SimpleChanges,
  ViewContainerRef,
  Injector, EnvironmentInjector, Renderer2, afterNextRender
} from '@angular/core';
...
import {
  ...
  ScrollStrategy,
  ConnectedPosition,
  createFlexibleConnectedPositionStrategy, createRepositionScrollStrategy, createOverlayRef,
...
import {CUTE_FORM_FIELD, CuteFormField} from '@cute-widgets/base/form-field';
import {defer, EMPTY, merge, Observable, of as observableOf, Subject, Subscription} from 'rxjs';
...
import {
  CuteAutocompleteDefaultOptions,
  CUTE_AUTOCOMPLETE_DEFAULT_OPTIONS,
  CuteAutocomplete,
} from './autocomplete';
 ...
 * @docs-private
 */
export const CUTE_AUTOCOMPLETE_VALUE_ACCESSOR: any = {
 ...
 * @docs-private
 */
export function getCuteAutocompleteMissingPanelError(): Error {
  return Error(
    'Attempting to open an undefined instance of `cute-autocomplete`. ' +
...
export const CUTE_AUTOCOMPLETE_SCROLL_STRATEGY = new InjectionToken<() => ScrollStrategy>(
  'cute-autocomplete-scroll-strategy',
  {
    providedIn: 'root',
    factory: () => {
...
export class CuteAutocompleteTrigger
  implements ControlValueAccessor, AfterViewInit, OnChanges, OnDestroy, Expandable
{
  ...
  private _formField = inject<CuteFormField | null>(CUTE_FORM_FIELD, {optional: true, host: true});
  private _viewportRuler = inject(ViewportRuler);
  private _scrollStrategy = inject(CUTE_AUTOCOMPLETE_SCROLL_STRATEGY);
  private _renderer = inject(Renderer2);
  private _animationsDisabled = _animationsDisabled();
  private _defaults = inject<CuteAutocompleteDefaultOptions | null>(
    CUTE_AUTOCOMPLETE_DEFAULT_OPTIONS,
    {optional: true},
  );
  ...
  private _closingActionsSubscription: Subscription | undefined;
   ...
   */
  private _pendingAutoselectedOption: CuteOption | null = null;
  ...
  @Input('cuteAutocomplete') autocomplete!: CuteAutocomplete;
   ...
   */
  @Input('cuteAutocompletePosition') position: 'auto' | 'above' | 'below' = 'auto';

  /**
   * Reference relative to which to position the autocomplete panel.
   * Defaults to the autocomplete trigger element.
   */
  @Input('cuteAutocompleteConnectedTo') connectedTo: CuteAutocompleteOrigin | undefined;
   ...
   */
  @Input({alias: 'cuteAutocompleteDisabled', transform: booleanAttribute})
  ...
  private _aboveClass = 'cute-autocomplete-panel-above';
  ...
  toggle(): void { this.panelOpen ? this.closePanel() : this.openPanel() }
  ...
  get panelClosingActions(): Observable<CuteOptionSelectionChange | null> {
    return merge(
      ...
      this.autocomplete._keyManager?.tabOut.pipe(filter(() => this._overlayAttached)) || EMPTY,
  ...
  readonly optionSelections: Observable<CuteOptionSelectionChange> = defer(() => {
   ...
   */
  private _setValueAndClose(event: CuteOptionSelectionChange | null): void {
   ...
   */
  private _clearPreviousSelectedOption(skip: CuteOption | null, emitEvent?: boolean) {
